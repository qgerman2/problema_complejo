...
path = [[Resources\\plugins\\FlyWithLua\\Scripts\\csv_player]]

coro = false -- corutina actual
filepath = false -- archivo csv actual
...
for file in lfs.dir(path) do
    if string.sub(file, -4) == ".csv" then
        add_macro( file, [[
            filepath = "]] .. path .. [[\\]] .. file .. [["
            coro = coroutine.create(process)]])
    end
end

function process()
    print(filepath)
    local f = csv.open(filepath)
    -- datarefs de entrada
        ...
        if i > 0 then
            ...
            if i > 1 then
                while true do
                    ...
                    graphics.draw_rectangle(SCREEN_WIDTH - 280, SCREEN_HEIGHT - 200, SCREEN_WIDTH - 10, SCREEN_HEIGHT - 60)
                    -- circulos
                    ...
                    graphics.draw_line(SCREEN_WIDTH - 210, SCREEN_HEIGHT - 130 - 60, SCREEN_WIDTH - 210, SCREEN_HEIGHT - 130 + 60)
                    graphics.draw_line(SCREEN_WIDTH - 80, SCREEN_HEIGHT - 130 - 60, SCREEN_WIDTH - 80, SCREEN_HEIGHT - 130 + 60)
                    graphics.draw_line(SCREEN_WIDTH - 210 - 60, SCREEN_HEIGHT - 130, SCREEN_WIDTH - 210 + 60, SCREEN_HEIGHT - 130)
                    graphics.draw_line(SCREEN_WIDTH - 80 - 60, SCREEN_HEIGHT - 130, SCREEN_WIDTH - 80 + 60, SCREEN_HEIGHT - 130)
                    -- entrada
